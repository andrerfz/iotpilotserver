...
export const GET = withCustomerContext(async (request: NextRequest) => {
  try {
    ...
    const dbMetrics = await getDatabaseMetrics(currentUser.isSuperAdmin);

    // Get application metrics
    const appMetrics = await getApplicationMetrics(currentUser.isSuperAdmin);
...
async function getDatabaseMetrics(isSuperAdmin: boolean) {
  try {
    ...
    const [
      ...
      isSuperAdmin ? tenantPrisma.client.customer.count() : Promise.resolve(1)
    ]);
