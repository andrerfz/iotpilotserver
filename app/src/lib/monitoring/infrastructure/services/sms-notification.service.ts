...
export class SMSNotificationService {
  ...
  private createAlertMessage(alert: Alert): string {
    ...
    const metricName = alert.metadata?.metricName || 'N/A';
    const metricValue = alert.metadata?.metricValue || { value: 'N/A', unit: '' };
    const thresholdValue = alert.metadata?.thresholdValue || 'N/A';
    
    return `${severityPrefix} ALERT: ${alert.title}\n` +
           `Device: ${alert.deviceId}\n` +
           `Time: ${timestamp}\n` +
           `Metric: ${metricName}\n` +
           `Value: ${metricValue.value} ${metricValue.unit}\n` +
           `Threshold: ${thresholdValue} ${metricValue.unit}\n\n` +
           `View in IoT Pilot: ${this.dashboardUrl}/devices/${alert.deviceId}`;
  }
