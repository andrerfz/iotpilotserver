...
import {DeviceEntity, SSHCredentials} from '@/lib/device/domain/entities/device.entity';
...
export class RegisterDeviceCompleteHandler implements CommandHandler<RegisterDeviceCompleteCommand, DeviceRegistrationResult> {
  ...
  async handle(command: RegisterDeviceCompleteCommand): Promise<DeviceRegistrationResult> {
    ...
    try {
      ...
      // Additional device properties are handled at the Prisma level
      // The current Device entity only supports basic properties (id, name, ipAddress, status, sshCredentials)
