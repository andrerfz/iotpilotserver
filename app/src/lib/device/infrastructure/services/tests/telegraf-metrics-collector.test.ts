...
describe('TelegrafMetricsCollector', () => {
  ...
  describe('constructor', () => {
    it('should initialize InfluxDB client with correct parameters', () => {
  ...
  describe('collectMetrics', () => {
    it('should collect metrics successfully from InfluxDB', async () => {
      ...
      expect(result.networkUpload).toBe(1024000);
      expect(result.networkDownload).toBe(2048000);
    ...
    it('should handle InfluxDB query errors', async () => {
      ...
      await expect(collector.collectMetrics(deviceId))
        .rejects.toThrow('Failed to collect metrics from InfluxDB: InfluxDB connection failed');
    });
  ...
  describe('writeMetrics', () => {
    it('should write metrics to InfluxDB successfully', async () => {
      const metrics = {
        cpuUsage: 15.5,
        memoryUsage: 22.3,
        diskUsage: 45.0,
