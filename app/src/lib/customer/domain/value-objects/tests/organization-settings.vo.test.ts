...
describe('OrganizationSettings Value Object', () => {
    describe('constructor', () => {
        ...
        it('should create OrganizationSettings with provided values', () => {
            ...
            const settings = new OrganizationSettings(props);
        ...
        it('should throw error for maxUsers less than 1', () => {
            expect(() => new OrganizationSettings({ maxUsers: 0 })).toThrow('Maximum users must be at least 1');
        });

        it('should throw error for maxDevices less than 1', () => {
            expect(() => new OrganizationSettings({ maxDevices: 0 })).toThrow('Maximum devices must be at least 1');
        });

        it('should throw error for dataRetentionDays less than 1', () => {
            expect(() => new OrganizationSettings({ dataRetentionDays: 0 })).toThrow('Data retention days must be at least 1');
        });

        it('should throw error for invalid custom domain format', () => {
            expect(() => new OrganizationSettings({ customDomain: 'invalid-domain' })).toThrow('Invalid custom domain format');
        });
    });

    describe('create static method', () => {
        it('should create OrganizationSettings with provided parameters', () => {
            const settings = OrganizationSettings.create(20, 100, ['basic', 'advanced'], 'default', 'test.com');
        ...
        it('should use default values when parameters not provided', () => {
            const settings = OrganizationSettings.create(15, 75);
    ...
    describe('equals', () => {
        it('should return true for equal OrganizationSettings', () => {
            const settings1 = new OrganizationSettings({
                maxUsers: 100,
                maxDevices: 500,
                allowedFeatures: ['basic', 'advanced'],
                dataRetentionDays: 90,
            ...
            const settings2 = new OrganizationSettings({
                maxUsers: 100,
                maxDevices: 500,
                allowedFeatures: ['basic', 'advanced'],
                dataRetentionDays: 90,
        ...
        it('should return false for different maxUsers', () => {
            const settings1 = new OrganizationSettings({ maxUsers: 100 });
            const settings2 = new OrganizationSettings({ maxUsers: 200 });

            expect(settings1.equals(settings2)).toBe(false);
        });

        it('should return false for different maxDevices', () => {
            const settings1 = new OrganizationSettings({ maxDevices: 100 });
            const settings2 = new OrganizationSettings({ maxDevices: 200 });

            expect(settings1.equals(settings2)).toBe(false);
        });

        it('should return false for different allowedFeatures', () => {
            const settings1 = new OrganizationSettings({ allowedFeatures: ['basic'] });
            const settings2 = new OrganizationSettings({ allowedFeatures: ['advanced'] });

            expect(settings1.equals(settings2)).toBe(false);
        });

        it('should return false for different dataRetentionDays', () => {
            const settings1 = new OrganizationSettings({ dataRetentionDays: 30 });
            const settings2 = new OrganizationSettings({ dataRetentionDays: 90 });

            expect(settings1.equals(settings2)).toBe(false);
        });

        it('should return false for different customDomain', () => {
            const settings1 = new OrganizationSettings({ customDomain: 'example.com' });
            const settings2 = new OrganizationSettings({ customDomain: 'test.com' });

            expect(settings1.equals(settings2)).toBe(false);
        });

        it('should return false for different logoUrl', () => {
            const settings1 = new OrganizationSettings({ logoUrl: 'url1' });
            const settings2 = new OrganizationSettings({ logoUrl: 'url2' });

            expect(settings1.equals(settings2)).toBe(false);
        });

        it('should return false for different primaryColor', () => {
            const settings1 = new OrganizationSettings({ primaryColor: '#FF0000' });
            const settings2 = new OrganizationSettings({ primaryColor: '#00FF00' });

            expect(settings1.equals(settings2)).toBe(false);
        });

        it('should return false for different secondaryColor', () => {
            const settings1 = new OrganizationSettings({ secondaryColor: '#FF0000' });
            const settings2 = new OrganizationSettings({ secondaryColor: '#00FF00' });

            expect(settings1.equals(settings2)).toBe(false);
        });

        it('should return false for non-OrganizationSettings objects', () => {
            const settings = new OrganizationSettings({ maxUsers: 100 });
    ...
    describe('toObject', () => {
        it('should return a plain object representation', () => {
            ...
            expect(obj).toEqual({
                ...
                logoUrl: 'https://example.com/logo.png',
                primaryColor: '#FF0000',
                secondaryColor: 'secondaryColor' in props ? '#00FF00' : undefined
            });
        });

        it('should return undefined for null values in toObject', () => {
            const settings = new OrganizationSettings({});
            const obj = settings.toObject();

            expect(obj.customDomain).toBeUndefined();
            expect(obj.logoUrl).toBeUndefined();
            expect(obj.primaryColor).toBeUndefined();
            expect(obj.secondaryColor).toBeUndefined();
        });
    });
    ...
    describe('domain validation', () => {
        it('should accept valid domain formats', () => {
            ...
            validDomains.forEach(domain => {
                expect(() => new OrganizationSettings({ customDomain: domain })).not.toThrow();
            });
        });
        ...
        it('should reject invalid domain formats', () => {
            ...
            invalidDomains.forEach(domain => {
                expect(() => new OrganizationSettings({ customDomain: domain })).toThrow('Invalid custom domain format');
            });
        });
    });
});
