...
class InMemoryDeviceRepository implements DeviceRepository {
    ...
    async findById(id: DeviceId, customerId?: CustomerId): Promise<Device | null> {
        const device = this.devices.get(id.getValue());
        if (!device) return null;
        
        // Apply tenant filtering if customerId provided
        if (customerId && device.getTenantId().getValue() !== customerId.getValue()) {
            return null;
        }
    ...
    async findByDeviceId(deviceId: string, customerId?: CustomerId): Promise<Device | null> {
    ...
    async findAll(customerId?: CustomerId, filters?: any): Promise<Device[]> {
        let devices = Array.from(this.devices.values());
        
        // Apply tenant filtering
        if (customerId) {
            devices = devices.filter(d => d.getTenantId().getValue() === customerId.getValue());
        }
        
        // Apply status filter if provided
        if (filters?.status && filters.status !== 'all') {
            devices = devices.filter(d => d.status.getValue() === filters.status);
        }
        
        return devices;
    }

    async save(device: Device, customerId?: CustomerId): Promise<void> {
    ...
    async count(customerId?: CustomerId): Promise<number> {
        ...
        let count = 0;
        for (const device of this.devices.values()) {
            if (device.getTenantId().getValue() === customerId.getValue()) {
                count++;
            }
        }
        return count;
...
describe('Device Management Flow Integration', () => {
    ...
    beforeEach(() => {
        ...
        customerId1 = CustomerId.create('customer-1');
        customerId2 = CustomerId.create('customer-2');
    });
    ...
    describe('Device Registration and Retrieval', () => {
        it('should register a device and retrieve it', async () => {
            ...
            expect(retrievedDevice?.getTenantId().getValue()).toBe(customerId1.getValue());
        });
        ...
        it('should update device status', async () => {
            ...
            device.updateStatus(DeviceStatus.create('inactive'));
            await deviceRepository.save(device, customerId1);

            // Verify update
            const updatedDevice = await deviceRepository.findById(deviceId, customerId1);
            expect(updatedDevice?.status.getValue()).toBe('inactive');
        });
    ...
    describe('Multi-Tenant Isolation', () => {
        ...
        it('should prevent cross-tenant device updates', async () => {
            ...
            // Tenant 2 tries to update tenant 1's device
            await expect(deviceRepository.save(device, customerId2))
        ...
        it('should prevent cross-tenant device deletion', async () => {
            ...
            // Tenant 2 tries to delete tenant 1's device
            await expect(deviceRepository.delete(deviceId, customerId2))
    ...
    describe('Device Filtering and Queries', () => {
        beforeEach(async () => {
            ...
            for (let i = 0; i < statuses.length; i++) {
                ...
                device.updateStatus(DeviceStatus.create(statuses[i]));
                await deviceRepository.save(device, customerId1);
            }
        });
