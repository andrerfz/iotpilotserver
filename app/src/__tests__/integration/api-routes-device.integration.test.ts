...
describe('Device API Routes Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        timestamp = Date.now();
        const testCustomerIdUnique = `test-customer-device-api-${timestamp}`;
        const testUserIdUnique = `test-user-device-api-${timestamp}`;
        
        // Create test customer
        const testCustomer = await prismaService.getClient().customer.create({
            data: {
                id: testCustomerIdUnique,
    ...
    describe('POST /api/devices', () => {
        it('should register a new device successfully', async () => {
            ...
            expect(response.status).toBe(200);
            expect(data.device.deviceId).toBe('device_register_001');
            expect(data.device.hostname).toBe('test-device-register');
        });
    ...
    describe('GET /api/devices/[id]', () => {
        ...
        it('should get device details successfully', async () => {
            ...
            expect(response.status).toBe(200);
            expect(data.deviceId).toBe('device_get_001');
            expect(data.hostname).toBe('Test Device Get');
        });
    ...
    describe('PUT /api/devices/[id]', () => {
        ...
        it('should update device successfully', async () => {
            ...
            expect(response.status).toBe(200);
            expect(data.device.hostname).toBe('Updated Device Name');
        });
    });
    ...
    describe('POST /api/devices/bulk', () => {
        it('should register multiple devices successfully', async () => {
            const bulkData = {
                devices: [
                    {
                        name: 'bulk-device-1',
