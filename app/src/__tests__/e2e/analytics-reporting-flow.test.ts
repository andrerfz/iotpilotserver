...
describe('E2E: Analytics Reporting Flow', () => {
    ...
    describe('Historical Data Collection', () => {
        it('should collect comprehensive historical metrics data', async () => {
            ...
            const totalMetrics = await prisma.deviceMetrics.count({
                where: { customerId: customerDbId }
            });

            expect(totalMetrics).toBe(dataPoints * deviceIds.length);
        ...
        it('should generate historical alert data', async () => {
            ...
            // Generate alerts based on historical data
            const highCpuMetrics = await prisma.deviceMetrics.findMany({
                where: {
                    customerId: customerDbId,
                    cpuUsage: { gt: 70.0 }
                }
            ...
            for (const metric of highCpuMetrics.slice(0, 10)) { // Create 10 alerts
                await prisma.alert.create({
                    data: {
                        ...
                        message: `CPU usage exceeded threshold: ${metric.cpuUsage}%`,
    ...
    describe('Analytics Dashboard Data', () => {
        it('should generate comprehensive dashboard analytics', async () => {
    ...
    describe('Report Generation', () => {
        ...
        it('should generate time-based analytical reports', async () => {
                ...
                SELECT
                    json_build_object(
                        'summary', json_build_object(
                            'total_devices', (SELECT COUNT(*) FROM device WHERE "customerId" = ${customerDbId}),
                            'total_metrics', (SELECT COUNT(*) FROM "deviceMetrics" WHERE "customerId" = ${customerDbId}),
                            'total_alerts', (SELECT COUNT(*) FROM alert WHERE "customerId" = ${customerDbId}),
