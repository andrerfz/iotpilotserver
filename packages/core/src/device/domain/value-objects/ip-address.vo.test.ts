...
describe('IpAddress Value Object', () => {
  ...
  describe('create - invalid addresses', () => {
    ...
    it('should reject invalid IPv4 - octet out of range', () => {
      // Arrange, Act & Assert
      // The implementation validates format first, so it throws "Invalid IP address format"
      expect(() => IpAddress.create('192.168.1.256')).toThrow();
      expect(() => IpAddress.create('300.168.1.1')).toThrow();
    });
  ...
  describe('edge cases', () => {
    it('should handle leading zeros in IPv4', () => {
      // Arrange, Act & Assert
      // Leading zeros should be rejected as they can be ambiguous
      // The regex validation will catch this and throw "Invalid IP address format"
      expect(() => IpAddress.create('192.168.001.001')).toThrow();
    });

    it('should reject whitespace', () => {
      // Arrange, Act & Assert
      // The implementation trims whitespace, so ' 192.168.1.1 ' becomes '192.168.1.1' which is valid
      // But if there's whitespace in the middle, it will fail regex validation
      expect(() => IpAddress.create('192.168.1. 1')).toThrow();
      expect(() => IpAddress.create('192.168. 1.1')).toThrow();
    });

    it('should reject special characters', () => {
      // Arrange, Act & Assert
      // Special characters will fail regex validation
      // Note: The implementation trims, so trailing newlines are removed
      expect(() => IpAddress.create('192.168.1.1;DROP TABLE')).toThrow();
      expect(() => IpAddress.create('192.168.1.1@domain')).toThrow();
    });
  });
});
