...
import {ValueObjectValidationException} from '@iotpilot/core/shared/domain/exceptions/value-object-validation.exception';
...
export class DeviceStatus {
  ...
  // Maps Prisma/legacy status (ONLINE, OFFLINE, MAINTENANCE, ERROR) to domain DeviceStatus
  static createFromLegacyStatus(status: string | undefined, connectivity: ConnectivityStatus = 'offline', lastSeen?: Date): DeviceStatus {
    if (status == null || status === '') {
      return DeviceStatus.offlineInactive(lastSeen);
    }
    const statusMap: Record<string, { businessStatus: BusinessStatus; connectivity: ConnectivityStatus }> = {
      ONLINE: { businessStatus: 'active', connectivity: 'online' },
      OFFLINE: { businessStatus: 'inactive', connectivity: 'offline' },
      MAINTENANCE: { businessStatus: 'maintenance', connectivity: 'offline' },
      ERROR: { businessStatus: 'error', connectivity: 'offline' },
      UNCLAIMED: { businessStatus: 'inactive', connectivity: 'disconnected' },
      PENDING_SETUP: { businessStatus: 'inactive', connectivity: 'disconnected' },
    };
    const mapped = statusMap[status];
    if (mapped) {
      return new DeviceStatus({ ...mapped, lastSeen });
    }
    return new DeviceStatus({
      businessStatus: status as BusinessStatus,
      connectivity,
