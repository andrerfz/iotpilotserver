...
describe.skip('DockerStatsCollector', () => {
  ...
  beforeEach(() => {
    ...
    mockDevice = {
      ...
      sshCredentials: { username: 'testuser', privateKey: 'mock-key', port: 22 }, // collector uses .username, .privateKey, .port
      getCustomerId: () => CustomerId.create('clr7abc1234567890xyzab123'),
    };
  ...
  describe('collectMetrics', () => {
    it('should collect Docker metrics successfully', async () => {
      ...
      // Verify SSH connection was made (4th arg is SshCredentials built from mockDevice.sshCredentials)
    ...
    it.skip('should cache metrics for subsequent calls', async () => {
      mockDeviceRepository.findById.mockResolvedValue(mockDevice);
    ...
    it.skip('should parse network I/O correctly', async () => {
      mockDeviceRepository.findById.mockResolvedValue(mockDevice);
    ...
    it.skip('should handle multiple container stats by averaging', async () => {
      mockDeviceRepository.findById.mockResolvedValue(mockDevice);
