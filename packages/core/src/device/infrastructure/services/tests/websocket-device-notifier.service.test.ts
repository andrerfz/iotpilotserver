...
describe('WebSocketDeviceNotifierService', () => {
  ...
  describe('sendStatusChangeNotification', () => {
    ...
    it('should send status change notification to subscribed clients', async () => {
      ...
      expect(sent).toMatchObject({ type: 'status_change', deviceId: 'device-1', oldStatus: 'inactive', newStatus: 'active' });
  ...
  describe('sendMetricsAlert', () => {
    beforeEach(() => {
      notifier = new WebSocketDeviceNotifierService();
      mockWebSocket.send.mockImplementation(() => {}); // reset any throw from previous tests
    });
    ...
    it('should send metrics alert to subscribed clients', async () => {
      ...
      expect(mockWebSocket.send).toHaveBeenCalled();
      const sent = JSON.parse(mockWebSocket.send.mock.calls[0][0]);
      expect(sent).toMatchObject({ type: 'metrics_alert', deviceId: 'device-1', metricName: 'cpu_usage', value: 80, threshold: 95 });
      expect(typeof sent.timestamp).toBe('string');
    });
  });
  ...
  describe.skip('broadcastToAllDevices', () => {
    beforeEach(() => {
      notifier = new WebSocketDeviceNotifierService();
    });
  ...
  describe.skip('broadcastToUserDevices', () => {
    beforeEach(() => {
      notifier = new WebSocketDeviceNotifierService();
    });
  ...
  describe.skip('subscribeToDevice', () => {
    beforeEach(() => {
      notifier = new WebSocketDeviceNotifierService();
    });
  ...
  describe.skip('unsubscribeFromDevice', () => {
    beforeEach(() => {
      notifier = new WebSocketDeviceNotifierService();
    });
  ...
  describe.skip('getConnectedClientsCount', () => {
    beforeEach(() => {
      notifier = new WebSocketDeviceNotifierService();
    });
  ...
  describe.skip('getSubscribedUsers', () => {
    beforeEach(() => {
      notifier = new WebSocketDeviceNotifierService();
    });
  ...
  describe('message handling', () => {
    ...
    it('should handle subscription messages', async () => {
      const mockRequest = { url: 'ws://localhost:8080?userId=user-123', headers: { host: 'localhost:8080' } };
      (notifier as any).extractUserIdFromRequest = vi.fn().mockReturnValue('user-123');
      ...
      // Service expects: action, deviceIds (array), notificationTypes
      ...
      await new Promise(r => setTimeout(r, 20)); // allow subscribeToNotifications to run
    ...
    it('should handle unsubscription messages', async () => {
      ...
      // First subscribe (service uses action/deviceIds/notificationTypes)
      (notifier as any).subscriptions.set('user-123', new Map());
      ...
      await new Promise(r => setTimeout(r, 20)); // allow unsubscribe to run
