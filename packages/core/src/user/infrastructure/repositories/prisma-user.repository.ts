...
export class PrismaUserRepository implements UserRepository {
    ...
    async findByCustomerId(customerId: CustomerId): Promise<UserEntity[]> {
        return this.findAllInTenant(customerId);
    }
    ...
    async findActive(): Promise<UserEntity[]> {
        ...
        return users.map((u: any) => this.userMapper.toDomain(u));
    }

    async findInactive(): Promise<UserEntity[]> {
        const users = await this.prisma.getClient().user.findMany({
    ...
    async exists(id: UserId, _tenantContext?: TenantContext): Promise<boolean> {
    ...
    async count(tenantContext?: TenantContext): Promise<number> {
        const where: any = {};
        if (tenantContext && !tenantContext.isSuperAdmin() && tenantContext.getCustomerId()) {
            where.customerId = tenantContext.getCustomerId()!.getValue();
        }
        return this.prisma.getClient().user.count({ where });
    }
    ...
    async save(user: UserEntity): Promise<void> {
        ...
        if (existingUser) {
            // User exists - perform update only. Prisma User update: email, username, password, role, status, profileImage, updatedAt, lastLoginAt, deletedAt
            ...
            // User doesn't exist - perform create only. Prisma User: id, email, username, password, role, status, profileImage, createdAt, updatedAt, lastLoginAt, deletedAt, customer
            ...
            // For non-SUPERADMIN users, connect customer via relation
            if (customerId && !user.isSuperAdmin()) {
                createData.customer = { connect: { id: customerId } };
            }

            console.log(`DEBUG: Creating user with customerId: ${customerId}`);
