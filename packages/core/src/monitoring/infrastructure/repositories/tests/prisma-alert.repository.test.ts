...
describe('PrismaAlertRepository', () => {
  ...
  beforeEach(() => {
    ...
    repository = new PrismaAlertRepository(mockPrismaService as any, mockTenantValidator);
  ...
  describe('findById', () => {
    it('should return alert when found', async () => {
      ...
      const result = await repository.findById(AlertId.fromString('alert-1'), tenantId);

      expect(result).toBeInstanceOf(Alert);
      expect(result?.getId().getValue()).toBe('alert-1');
    ...
    it('should return null when alert not found', async () => {
      ...
      const result = await repository.findById(AlertId.fromString('non-existent'), tenantId);
    ...
    it('should throw error when tenant validation fails', async () => {
      ...
      await expect(repository.findById(AlertId.fromString('alert-1'), tenantId)).rejects.toThrow('Tenant access denied');
    });
  });
  ...
  describe('delete', () => {
    it('should delete alert', async () => {
      ...
      await repository.delete(AlertId.fromString('alert-1'), tenantId);
    ...
    it('should throw error when tenant validation fails', async () => {
      ...
      await expect(repository.delete(AlertId.fromString('alert-1'), tenantId)).rejects.toThrow('Tenant access denied');
    });
  });
