...
/** Creates a TenantContext-like object for tests. withTenant uses getCustomerId/getUserId/isSuperAdmin; tenant-prisma resolves from those when present. */
function createTestTenantContext(opts: {
    customerId: string | null;
    userId: string;
    isSuperAdmin: boolean;
}): TenantContext {
    return {
        getCustomerId: () => (opts.customerId ? { getValue: () => opts.customerId } : null),
        getUserId: () => ({ getValue: () => opts.userId }),
        getUserRole: () => ({} as UserRole),
        getRole: () => ({} as UserRole),
        isSuperAdmin: () => opts.isSuperAdmin,
        isSuperAdminUser: () => opts.isSuperAdmin,
        hasPermission: () => false,
        getTenantId: () => (opts.customerId ? { getValue: () => opts.customerId } : null),
        isMultiTenant: () => !!opts.customerId,
        getCorrelationId: () => 'test-correlation',
        getRequestId: () => 'test-request',
        getUserAgent: () => null,
        getClientIp: () => null,
        hasAccess: () => false,
        canBypassTenantRestrictions: () => opts.isSuperAdmin,
        requiresTenantScope: () => !opts.isSuperAdmin
    } as TenantContext;
}
...
describe('Tenant Isolation Security Tests', () => {
    ...
    beforeEach(async () => {
        ...
        // Create test contexts (must have getCustomerId/getUserId/isSuperAdmin for withTenant, and .customerId/.isSuperAdmin for tenant-prisma)
        tenantA = createTestTenantContext({ customerId: 'tenant-a-123', userId: 'user-a-123', isSuperAdmin: false });
        tenantB = createTestTenantContext({ customerId: 'tenant-b-456', userId: 'user-b-456', isSuperAdmin: false });
        superAdmin = createTestTenantContext({ customerId: null, userId: 'admin-123', isSuperAdmin: true });
    ...
    describe('SUPERADMIN Bypass Functionality', () => {
        it('should allow SUPERADMIN to access all customers without tenant filtering', async () => {
            ...
            expect(callArg == null || (typeof callArg === 'object' && !(callArg as any)?.where?.customerId)).toBe(true);
        ...
        it('should allow SUPERADMIN to access all users including other tenants', async () => {
            ...
            expect(callArg == null || (typeof callArg === 'object' && !(callArg as any)?.where?.AND)).toBe(true);
    ...
    describe('Tenant Boundary Violations', () => {
        ...
        it('should prevent tenant A from updating tenant B devices', async () => {
            ...
            expect(mockPrisma.device.update).toHaveBeenCalledWith(
                expect.objectContaining({
                    where: { id: 'device-123', customerId: 'tenant-a-123' },
                    data: { hostname: 'hacked-device' }
                })
            );
        });
        ...
        it('should prevent tenant A from deleting tenant B users', async () => {
            ...
            const call = mockPrisma.user.delete.mock.calls[0]?.[0];
            expect(call?.where?.AND).toBeDefined();
            expect(call?.where?.AND).toEqual(
                expect.arrayContaining([expect.objectContaining({ customerId: 'tenant-a-123' })])
            );
        });
    ...
    describe('Security Violations Without Context', () => {
        it('should throw security violation for device access without context', async () => {
            expect(() => tenantPrisma.client.device.findMany()).toThrow('SECURITY VIOLATION');
        });
    ...
    describe('Customer Model Access Restrictions', () => {
        ...
        it('should not inject customerId into customer create (customer is tenant root)', async () => {
