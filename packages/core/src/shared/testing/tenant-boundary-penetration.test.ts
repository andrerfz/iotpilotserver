...
/** Plain-object adapter so withTenant receives getCustomerId/getUserId/isSuperAdmin. */
function createTestContext(customerId: string | null, userId: string, _role: UserRoleType, isSuperAdmin: boolean): TenantContext {
  return {
    getCustomerId: () => customerId ? { getValue: () => customerId } : null,
    getUserId: () => ({ getValue: () => userId }),
    getUserRole: () => ({ getValue: () => 'USER' as UserRoleType }),
    getRole: () => ({ getValue: () => 'USER' as UserRoleType }),
    isSuperAdmin: () => isSuperAdmin,
    isSuperAdminUser: () => isSuperAdmin,
    hasPermission: () => false,
    getTenantId: () => customerId ? { getValue: () => customerId } : null,
    isMultiTenant: () => !!customerId,
    getCorrelationId: () => 'test',
    getRequestId: () => 'test',
    getUserAgent: () => null,
    getClientIp: () => null,
    hasAccess: () => false,
    canBypassTenantRestrictions: () => isSuperAdmin,
...
describe('Tenant Boundary Penetration Tests', () => {
    ...
    beforeEach(() => {
        ...
        // Setup attacker and victim tenants (withTenant needs getCustomerId/getUserId/isSuperAdmin)
        attackerTenant = createTestContext('attacker-tenant-123', 'attacker-user-456', 'USER', false);
        victimTenant = createTestContext('victim-tenant-789', 'victim-user-101', 'USER', false);
        superAdmin = createTestContext(null, 'admin-999', 'SUPERADMIN', true);
    });
    ...
    describe('Privilege Escalation Attempts', () => {
        it.skip('should prevent regular user from modifying SUPERADMIN accounts', async () => {
        ...
        it.skip('should prevent regular user from deleting SUPERADMIN accounts', async () => {
        ...
        it.skip('should prevent user from elevating their own privileges', async () => {
    ...
    describe('Context Poisoning Attacks', () => {
        it('should prevent context poisoning through malformed tenant data', async () => {
            // Attacker tries to inject malicious context (must still have getCustomerId etc. for withTenant)
            const poisonedContext = createTestContext('victim-tenant-789', 'attacker-user-456', 'USER', false);
        ...
        it.skip('should handle null/undefined tenant context gracefully', async () => {
    ...
    describe('Information Disclosure Prevention', () => {
        ...
        it.skip('should prevent timing attacks through error response times', async () => {
