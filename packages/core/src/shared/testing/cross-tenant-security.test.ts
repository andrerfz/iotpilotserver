...
import {AuditEventType, TenantAuditLogger} from '@iotpilot/core/shared/infrastructure/security/tenant-audit-logger';
import {CustomerRepository} from '@iotpilot/core/customer/infrastructure/persistence/customer.repository';
import {TenantDataMigrationUtils} from '@iotpilot/core/shared/infrastructure/migration/tenant-data-migration.utils';
...
describe.skip('Cross-Tenant Security', () => {
    ...
    describe('Cross-Tenant Access Prevention', () => {
        ...
        it('should prevent updating entities from another tenant', async () => {
            ...
            await fixture.tenantTestUtils.runWithTenantContext(adminContext, async () => {
                ...
                console.log('Raw customer data:', rawCustomer);
            });
            ...
            await fixture.tenantTestUtils.runWithTenantContext(tenant2Context, async () => {
                ...
                try {
                    // This should fail because tenant2 shouldn't be able to update tenant1's data
                    ...
                    expect(true).toBe(false, 'Tenant 2 should not be able to update Tenant 1 data');
                } catch (error) {
                    // Expect an error because tenant 2 shouldn't be able to update tenant 1's data
                    expect(error).toBeDefined();
                    console.log('Expected error when tenant2 tries to update tenant1 data:', error.message);
                }
            });
            ...
            await fixture.tenantTestUtils.runWithTenantContext(adminContext, async () => {
                ...
                console.log('Original name:', rawCustomer.name);
    ...
    describe('SUPERADMIN Tenant Bypass', () => {
        it('should allow SUPERADMIN to access data across tenants', async () => {
            ...
            const customer2 = CustomerTestFactory.createCustomer(
                fixture.tenantIds[1].getValue(), // Use the tenant2 ID for the customer
                'Tenant 2 Customer'
            );
    ...
    describe('Audit Logging for Security Events', () => {
        it('should log cross-tenant access attempts', async () => {
            ...
            const logSpy = vi.spyOn(loggingService, 'error');
        ...
        it('should log SUPERADMIN actions', async () => {
            ...
            const auditSpy = vi.spyOn(tenantAuditLogger, 'logAuditEvent');
