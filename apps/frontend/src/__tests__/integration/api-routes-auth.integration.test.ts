...
let customersByDomain: Record<string, string> = {}; // domain -> customerId mapping
const mockApi = {
  register: vi.fn().mockImplementation((data: { email: string; username: string; password: string }) => {
    ...
    // Check if customer exists for this domain
    let customerId: string;
    let isNewCompany: boolean;
    
    if (customersByDomain[emailDomain]) {
      // Existing customer domain
      customerId = customersByDomain[emailDomain];
      isNewCompany = false;
    } else {
      // New customer domain
      customerId = 'test_customer_auth_api_' + Date.now();
      customersByDomain[emailDomain] = customerId;
      isNewCompany = true;
    }
...
describe('Auth API Routes Integration Tests', () => {
  beforeAll(() => {
    ...
    customersByDomain = {}; // Reset customer domains before each test
    });
    ...
    describe('POST /api/auth/register', () => {
      ...
      expect(data.user.customerId).toBeDefined();
            expect(data.isNewCompany).toBe(true); // First registration for this domain creates new company
        });
    ...
    describe('Multi-tenant Authentication', () => {
      beforeEach(async () => {
        ...
        it('should allow users to register for existing customer domain', async () => {
            // First, register a user to create the customer domain
            const firstUserData = {
          email: `firstuser@testauthapi.com`,
                username: 'firstuser',
                password: 'TestPassword123!'
            };
            
            const firstResponse = mockApi.register(firstUserData);
            expect(firstResponse.status).toBe(200);
            expect(firstResponse.body.isNewCompany).toBe(true); // First user creates company
            
            const customerId = firstResponse.body.user.customerId;
            
            // Now register a second user with the same domain
            const secondUserData = {
          email: `seconduser@testauthapi.com`, // Same domain
                username: 'seconduser',
                password: 'TestPassword123!'
            };

        const response = mockApi.register(secondUserData);
        const data = response.body;

            expect(response.status).toBe(200);
        expect(data.user.customerId).toBe(customerId); // Should join same customer
            expect(data.user.role).toBe('ADMIN'); // First user for a customer gets ADMIN role
            expect(data.isNewCompany).toBe(false); // Joining existing company
        });
    });
