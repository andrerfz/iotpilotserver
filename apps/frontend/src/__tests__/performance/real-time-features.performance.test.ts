...
describe('Real-time Features Performance', () => {
    ...
    beforeAll(async () => {
        ...
        const customer = await prisma.customer.create({
            data: {
                name: `Real-time Performance Customer ${timestamp}`,
                slug: `realtime-performance-customer-${timestamp}`,
                status: 'ACTIVE'
            }
        });
    ...
    describe('WebSocket Connection Performance', () => {
        ...
        it('should handle concurrent WebSocket clients', async () => {
            ...
            expect(overallThroughput).toBeGreaterThanOrEqual(REALTIME_THRESHOLDS.websocket.messagesPerSecond * 0.2); // 20% of target for concurrent load in test environment
        });
    ...
    describe('MQTT Message Performance', () => {
        ...
        it('should handle MQTT topic subscriptions', async () => {
            ...
            expect(mqttClient.getStats().subscriptions).toBeGreaterThanOrEqual(subscriptionCount);
        });

        it.skip('should handle MQTT wildcard subscriptions and publishing', async () => {
        ...
        it('should handle MQTT connection stress testing', async () => {
            ...
            expect(overallThroughput).toBeGreaterThanOrEqual(REALTIME_THRESHOLDS.mqtt.messagesPerSecond * 0.08); // 8% of target; CI/local variance (400 ops/sec)
        });
    });
