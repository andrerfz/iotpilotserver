...
describe('Database Queries Stress Testing', () => {
    ...
    const DB_THRESHOLDS = {
        simpleQuery: 70,      // ms - simple queries; CI variance
        complexQuery: 1200,  // ms - complex aggregations (raw SQL, groupBy); Time-based Metrics can be ~1s
    ...
    beforeAll(async () => {
        ...
        const customer = await prisma.customer.create({
            data: {
                name: `Database Stress Test Customer ${timestamp}`,
                slug: `db-stress-test-customer-${timestamp}`,
                status: 'ACTIVE'
            }
        });
        ...
        // Reduced to 200 to avoid Prisma engine reference counting issues
        largeDatasetSize = 200; // 200 devices for stress testing (sufficient for stress testing)
            ...
            devicePromises.push(
                prisma.device.create({
                    data: {
                        ...
                        deviceType: ['PI_4', 'PI_3', 'ORANGE_PI'][i % 3],
        ...
        for (const deviceId of deviceIds.slice(0, 50)) { // Only first 50 devices for metrics
    ...
    afterAll(async () => {
        ...
        try {
            ...
            // Delete devices in batches to avoid overwhelming Prisma
            if (deviceIds.length > 0) {
                const batchSize = 100;
            ...
            // Explicitly disconnect Prisma to release all connections
    ...
    describe('Analytical Query Performance', () => {
        it('should handle complex analytical aggregations', async () => {
            const analyticalQueries = [
                ...
                },
                {
                    ...
                    query: () => prisma.$queryRaw`
                        SELECT
                            percentile_cont(0.5) WITHIN GROUP (ORDER BY value) as median_cpu,
        ...
        it('should handle large dataset aggregations efficiently', async () => {
            // Create even more metrics data for large-scale testing (60s timeout: 5k metrics + 2 heavy aggregations)
            // Process in batches to avoid connection pool exhaustion
            const batchSize = 100;
            const totalMetrics = 5000;
            
            for (let batch = 0; batch < totalMetrics; batch += batchSize) {
                const batchPromises = [];
                const end = Math.min(batch + batchSize, totalMetrics);
            ...
            const largeScaleQueries = [
                {
                    ...
                    query: () => prisma.$queryRaw`
                        SELECT
                            ...
                            AVG(CASE WHEN dm.metric = 'cpu_usage' THEN dm.value END) as avg_cpu,
                            MIN(CASE WHEN dm.metric = 'cpu_usage' THEN dm.value END) as min_cpu,
                            MAX(CASE WHEN dm.metric = 'cpu_usage' THEN dm.value END) as max_cpu,
                ...
                },
                {
                    ...
                    query: () => prisma.$queryRaw`
                        SELECT
                            ...
                            AVG(CASE WHEN dm.metric = 'cpu_usage' THEN dm.value END) as avg_cpu,
                            AVG(CASE WHEN dm.metric = 'memory_usage' THEN dm.value END) as avg_memory,
                            STDDEV(CASE WHEN dm.metric = 'cpu_usage' THEN dm.value END) as cpu_variance,
                            MAX(CASE WHEN dm.metric = 'cpu_usage' THEN dm.value END) as peak_cpu
                        FROM devices d
                        JOIN device_metrics dm ON d.id = dm."deviceId"
            ...
            // Clean up additional metrics (using deviceId since no customerId on DeviceMetric)
            if (deviceIds.length > 0) {
                await prisma.deviceMetric.deleteMany({
                    where: {
    ...
    describe('Memory Usage During Queries', () => {
        it('should monitor memory usage with large result sets', async () => {
            ...
            expect(largeResult[0].metrics).toBeDefined(); // Changed from deviceMetrics to metrics
        });
            ...
            const optimizationCandidates = [
                ...
                },
                {
                    ...
                    query: () => prisma.device.findMany({
                        where: { customerId: customerDbId },
                        include: {
                            metrics: { // Changed from deviceMetrics to metrics
                                take: 5,
                                orderBy: { timestamp: 'desc' }
                            },
                            alerts: {
