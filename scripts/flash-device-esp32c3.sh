#!/usr/bin/env bash
#
# Manufacturing flash tool for LILYGO TTGO T-OI Plus (ESP32-C3) temperature sensors.
#
# Usage:
...
#
# T-OI Plus uses CH340K (usbserial) or WCH (wchusbserial).
...
detect_port() {
    ...
    if [ -z "$ports" ]; then
        error "No USB serial device detected. Plug in the T-OI Plus and try again."
    fi
...
#
# The T-OI Plus auto-resets into bootloader via DTR/RTS toggling, which
...
compile_and_flash() {
    ...
    echo ""
    echo -e "  Next device: plug in a new T-OI Plus and run ${CYAN}make device-flash-esp32c3${NC}"
    echo ""
}
